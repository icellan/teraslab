...
// TestGoldenRawMetadata pins DecodeTxMetadataRaw against the shared 320-byte
...
func TestGoldenRawMetadata(t *testing.T) {
	if len(goldenRawMetadata) != 320 {
		t.Fatalf("golden vector length = %d, want 320", len(goldenRawMetadata))
	}
	if RawMetadataSize != 320 {
		t.Fatalf("RawMetadataSize = %d, want 320", RawMetadataSize)
	}
	...
	if raw.BlockEntryCount != 1 {
		t.Fatalf("BlockEntryCount = %d, want 1", raw.BlockEntryCount)
	}
	if raw.BlockOverflowOffset != 0x4444444444444444 {
		t.Fatalf("BlockOverflowOffset = %#x, want 0x4444444444444444", raw.BlockOverflowOffset)
	}
