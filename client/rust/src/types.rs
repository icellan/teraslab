...
impl TxMetadataRaw {
    ...
    pub fn decode(data: &[u8]) -> Result<Self, ClientError> {
        ...
        // block_entries_inline + block_overflow_offset) was removed — mined
        // state now lives only in the server's in-RAM MinedIndex.
        ...
        Ok(Self {
            ...
            reassignment_offset: u64::from_le_bytes(data[117..125].try_into().unwrap()),
            reassignment_count: data[125],
            conflicting_children_count: data[203],
            conflicting_children_offset: u64::from_le_bytes(data[204..212].try_into().unwrap()),
        })
    }
}
...
mod tests {
    ...
    //
    // This 256-byte vector is byte-for-byte identical to `GOLDEN_RAW_METADATA`
    ...
    #[test]
    fn raw_metadata_size_is_256() {
        assert_eq!(RAW_METADATA_SIZE, 256);
    }
}
