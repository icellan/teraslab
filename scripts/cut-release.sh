#!/usr/bin/env bash
# Cut a new release by creating and pushing a version tag.
#
# Usage:
...
echo "Trigger:  push tag -> .github/workflows/release.yml (test, build x4, docker push, GitHub Release)"

if [ "$DRY_RUN" -eq 1 ]; then
  echo "[dry-run] would: git tag -a $VERSION -m '...' && git push $REMOTE $VERSION"
  exit 0
fi
...
echo
echo "Pushed $VERSION. Watch the release build:"
