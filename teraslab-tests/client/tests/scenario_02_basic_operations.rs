...
async fn scenario_02_basic_operations() {
    ...
    let timeout_guard = tokio::time::timeout(Duration::from_secs(300), async {
        ...
        let (_docker, client) = common::start_3node_cluster(SID)
            .await
        ...
        let txids = common::seed_records(&client, &verifier, 1000, 10)
            .await
        ...
        for chunk in txids.chunks(50) {
            ...
            for (i, result) in results.iter().enumerate() {
                ...
                let (meta, _) = TxMetadata::decode(FIELD_ALL, result.data()).unwrap_or_else(|e| {
                    panic!(
        ...
        let multi_rec = verifier
            .get_record(&multi_txid)
            .expect("record should exist in verifier");

        // Read the spent_utxos before spending
        let pre_results = client
            .get_batch(FIELD_ALL_METADATA, &[multi_txid])
            .await
        ...
        let multi_items: Vec<SpendItem> = (0u32..5)
            ...
            .collect();
        client
            .spend_batch(&spend_params, &multi_items)
            .await
            .unwrap_or_else(|e| {
                panic!(
        ...
        eprintln!(
            "[2.3] OK -- spent vouts 0-4 on txid {}, counter={}",
            txid_hex(&multi_txid),
            post_meta.spent_utxos
        );
        ...
        let unspend_rec = verifier
            .get_record(&unspend_txid)
            .expect("Test 2.4: record should exist in verifier");

        // Verify it's currently spent (counter = 1)
        let pre_results = client
            .get_batch(FIELD_ALL_METADATA, &[unspend_txid])
            .await
        ...
        for txid in mined_txids.iter() {
            ...
            assert_eq!(
                ...
                0,
                "Test 2.5: txid {} should be readable",
                txid_hex(txid)
            );
        ...
        for chunk in unset_mined_txids.chunks(50) {
            client
                .set_mined_batch(&unset_mined_params, chunk)
                .await
        ...
        for txid in unset_mined_txids.iter() {
            ...
            assert_eq!(
                results.item(0).status,
                0,
                "Test 2.6: txid {} should be readable",
                txid_hex(txid)
            );
        ...
        eprintln!(
            "[2.7] Full freeze lifecycle: freeze -> spend fails FROZEN -> unfreeze -> spend succeeds"
        );

        let freeze_txids = common::seed_records(&client, &verifier, 1, 5)
            .await
            .expect("Test 2.7: failed to create freeze test record");
        let freeze_txid = freeze_txids[0];
        let freeze_rec = verifier
            .get_record(&freeze_txid)
        ...
        let frozen_spend_result = client
            .spend_batch(&spend_params, &[frozen_spend_item])
            .await;
        ...
        client
            .spend_batch(&spend_params, &[unfrozen_spend_item])
            .await
            .unwrap_or_else(|e| {
                panic!("Test 2.7: spending after unfreeze should succeed, but got: {e}")
            });
        verifier.record_spend(freeze_txid, 0);

        eprintln!(
            "[2.7] OK -- freeze lifecycle passed: freeze -> FROZEN error -> unfreeze -> spend OK"
        );
        ...
        let reassign_txids = common::seed_records(&client, &verifier, 1, 3)
            .await
            .expect("Test 2.8: failed to create reassign test record");
        let reassign_txid = reassign_txids[0];
        let reassign_rec = verifier
            .get_record(&reassign_txid)
        ...
        client
            .freeze_batch(&[reassign_freeze_item])
            .await
        ...
        let readback = client
            .get_batch(FIELD_ALL, std::slice::from_ref(&reassign_txid))
            .await
            .unwrap_or_else(|e| panic!("Test 2.8: read-back after reassign failed: {e}"));
        assert!(
            readback.found(0),
            "Test 2.8: reassigned txid should be readable"
        );
        ...
        let low_result = client
            .spend_batch(
                &low_height_params,
                std::slice::from_ref(&reassign_spend_item),
            )
            .await;
        match low_result {
            Err(ClientError::Partial(ref pe)) => {
        ...
        match client
            .spend_batch(&high_height_params, &[reassign_spend_item])
            .await
        {
            Ok(_) => {}
            Err(ClientError::Partial(ref pe)) => {
                for err in &pe.errors {
                    eprintln!(
                        "[2.8] spend error at height 11: item={} code={} data={:?}",
                        err.item_index, err.code, err.data
                    );
                }
                panic!(
                    "Test 2.8: spending at spendable_after height should succeed, got partial error"
                );
            }
            Err(e) => {
        ...
        eprintln!(
            "[2.8] OK -- reassign with spendableAfter succeeded, spend-before-height rejected, spend-at-height accepted"
        );
        ...
        client
            .set_conflicting_batch(&set_conflicting_params, &conflicting_txids)
            .await
        ...
        let conflict_rec = verifier
            .get_record(&conflict_test_txid)
        ...
        match conflict_result {
            Err(ClientError::Partial(ref pe)) => {
                assert!(
                    pe.errors
                        .iter()
                        .any(|e| e.code == teraslab::protocol::opcodes::ERR_CONFLICTING),
        ...
        client
            .spend_batch(&lenient_params, &[conflict_spend2])
            .await
            .unwrap_or_else(|e| {
                panic!("Test 2.9: spend with ignore_conflicting should succeed: {e}")
            });
        ...
        client
            .set_locked_batch(true, &locked_txids)
            .await
        ...
        let locked_rec = verifier
            .get_record(&locked_test_txid)
        ...
        match locked_result {
            Err(ClientError::Partial(ref pe)) => {
                assert!(
                    pe.errors
                        .iter()
                        .any(|e| e.code == teraslab::protocol::opcodes::ERR_LOCKED),
        ...
        client
            .set_mined_batch(&locked_mine_params, &[locked_test_txid])
            .await
        ...
        let locked_results = client
            .get_batch(FIELD_ALL_METADATA, &[locked_test_txid])
            .await
        ...
        client
            .preserve_until_batch(preserve_height, &preserve_txids)
            .await
        ...
        let immature_result = client
            .spend_batch(&immature_params, std::slice::from_ref(&coinbase_spend))
            .await;
        match immature_result {
            Err(ClientError::Partial(ref pe)) => {
                assert!(
                    pe.errors
                        .iter()
                        .any(|e| e.code == teraslab::protocol::opcodes::ERR_COINBASE_IMMATURE),
        ...
        client
            .spend_batch(&mature_params, &[coinbase_spend])
            .await
        ...
        let batch_create_txids = common::seed_records(&client, &verifier, 100, 5)
            .await
            .expect("Test 2.14: batch create failed");
        assert_eq!(
            batch_create_txids.len(),
            100,
            "Test 2.14: expected 100 txids"
        );

        // Verify all 100 are readable
        let results = client
            .get_batch(FIELD_ALL_METADATA, &batch_create_txids)
            .await
        ...
        for chunk in batch_get_txids.chunks(50) {
            ...
            for (i, result) in results.iter().enumerate() {
                ...
                if result.status() == 0
                    && let Some((spent_count, is_mined, is_conflicting, is_locked)) =
                        parse_metadata_fields(result.data())
                {
                    ...
                    assert!(
                        ...
                        txid_hex(&chunk[i]),
                        mm.iter()
                            .map(|m| format!("{}={} (expected {})", m.field, m.actual, m.expected))
                            .collect::<Vec<_>>(),
                    );
                }
                all_found += 1;
        ...
        assert_eq!(
            all_found, 200,
            "Test 2.15: should have found all 200 records"
        );
        ...
        let mismatches = common::verify_consistency(&client, &verifier)
            .await
