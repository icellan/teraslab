...
/// Task 16d: `SetMinedBatch` is no longer replayed against the device at
/// all (see `recovery.rs`'s doc comment on the `replay_one_recovery_entry`
/// match arm) — mined-state recovery is `Engine::replay_mined_index_redo_tail`'s
/// job now (Task 13), operating on the MinedIndex, which this
/// `recover_all_with_allocator`-level test does not construct. This pins
/// the device-level contract: `Create` still replays and reconstructs the
/// record; `SetMinedBatch` is a device-level no-op on every pass (first AND
/// repeated), so the device's block-entry fields and generation never move.
#[test]
fn boundary_set_mined_after_wal_replays_and_second_pass_is_idempotent() {
    ...
    assert_eq!(
        stats.entries_replayed, 1,
        "only create applies against the device; set_mined is a device-level no-op"
    );
    assert_eq!(
        stats.entries_skipped, 1,
        "set_mined is Skipped, not Applied"
    );
    assert_eq!(stats.entries_failed, 0);
    ...
    // index/secondaries, same device + WAL. Both entries must still be a
