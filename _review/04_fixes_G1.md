...
### F-G1-003 — FIXED (partial; deeper migration in follow-ups)
- Commit: pre-existing in WIP baseline (see fences + concurrent stress test at `src/io.rs:255-282` and `src/io.rs:889-990`)
- Files changed: `src/io.rs`
- Test added/extended: `direct_read_write_concurrent_stress_never_returns_torn_data` (in-crate `mod tests`)
- Notes: Acquire/Release fences in `read_metadata_direct` / `write_metadata_direct` (and the slot variants) are present with detailed BC-06/BC-07 comments and a concurrent stress test that asserts a CRC-clean read never returns a torn (tx_version, fee) combination. The Rust-strict-memory-model concern (fences alone do not establish happens-before without paired atomic load/store) is documented in the doc comment and the CRC remains the true safety net. Recommendation (a) — migrate metadata reads/writes to `AtomicU64` chunks for legal racing access — is a wider change touching every direct-path call site; filed in `_review/follow_ups.md`.

### F-G1-004 — DEFERRED
- Files: `src/device.rs::MemoryDevice` (the parallel `data: RwLock<Vec<u8>>` + `raw_ptr` aliasing the same heap allocation)
- Notes: Production never mixes the two paths so the bug is latent, but `cargo miri` against the test suite would flag the UB. The fix is either (a) drop `as_raw_ptr` for `MemoryDevice` and route everything through the lock guard, or (b) drop the lock and route every read/write through `raw_ptr`. Both interact with G2 callers and are larger than the surgical-fix budget. F-G1-017 (this round) removed the parallel `raw_len` field, so the remaining aliasing surface is the only outstanding piece. Filed in `_review/follow_ups.md`.
