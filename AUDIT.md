...
### Wave 3 — residual gaps + the four §E follow-ups (2026-06-11)
The §7 milestones above were complete after Wave 2; Wave 3 closed the three residual gaps and the four follow-ups that Wave 2 had logged.

| Item | Resolution |
|---|---|
| **J-03 residual** (real block device) | **Closed.** `tests/block_device_size.rs` attaches an unprivileged macOS RAM disk (`hdiutil attach -nomount ram://`), opens the real `S_IFBLK` node through `DirectDevice::open`, and asserts `size()` equals the ioctl-derived geometry (2 MiB), NOT the bogus open hint — so the kernel `DKIOCGETBLOCKCOUNT`/`DKIOCGETBLOCKSIZE` branch and the bounds check that consumes `actual_size` both run end-to-end. RAII detach guard, no leak across runs; clean runtime skip (not `#[ignore]`) where `hdiutil` is absent. Residual: Linux `BLKGETSIZE64` against a real `/dev/nvme` still needs a root loop-device CI job. |
| **N-04 residual** (deep fuzz) | **Demonstrated.** Ran `cargo-fuzz run decode_request` on nightly: **1,658,641 executions in 91 s, zero crashes**, 362-entry corpus. The scaffold works end-to-end and the wire decoder is panic-free over 1.66 M mutated inputs. The seeded smoke test remains the CI-enforced half; a periodic nightly cargo-fuzz job is the only remaining "wire it into CI" step. |
| **N-05 residual** (proxy delay/reorder) | **Closed.** The proxy fixture now injects per-directed-link seeded UDP delay+reorder (SplitMix64, fixed seed) and per-node inbound TCP delay, with live `tests/cluster_partition.rs` coverage (SWIM converges under heavy delay+reorder; detection-power + sabotage verified). Residual: per-link TCP attribution on loopback stays limited to the per-destination topology control plane (documented in `tests/net_proxy/mod.rs`). |
| **§E-1** receiver frame deadline | Ported the L-01 `DeadlineReader` to the replica receive path; extracted it to a shared `src/protocol/deadline.rs` (`pub(crate)`) used by both server and receiver. RED-before-fix drip test. **Also fixed a real latent bug found here**: the receiver's streaming-HMAC path chained `len_buf` ahead of the body, but `verify_signed_body_streaming` synthesises its own length prefix — every honest signed inter-node frame double-prefixed and failed `ERR_CLUSTER_AUTH_FAILED`. Latent (no production caller wires `auth_secret` yet) but a genuine inter-node-auth defect; fixed + pinned by `signed_replica_batch_verified_and_applied`. |
| **§E-2** split `cluster_secret` | Fail-closed validation guard: a typed `ConfigError::ClusterSecretMismatch` fires at server startup when clustering is active and the `ServerConfig`/`ClusterConfig` secrets disagree, instead of the previous silent cluster-formation hang. (A broader duplicate that also added a peak-quorum voter guard on `handle_commit` was **declined** — it introduced a transient legitimate-commit-rejection window under asymmetric peak observation, a behavior change inappropriate for a consensus path that E-4 said to keep.) |
| **§E-3** stale `client/rust` | Restored compilation + tests against current `teraslab` (added `cluster_id`/`swim_advertise_addr`, `strict_auth=false` in test servers, F-01 sort fix in the redirect test, bounded convergence poll). Confirmed `.github/workflows/ci.yml` already gates the crate via `--manifest-path`; documented why it stays out of the workspace. |
| **§E-4** quiesce voter trust | Document-and-pin (no behavior change): the fabricated-voter commit is annotated as trusting authenticated peers by design (cluster membership is the trust boundary; peak floor never lowers), with `quiesce_fabricated_commit_is_accepted_and_preserves_peak_floor` pinning the drain path so a future refactor can't silently break it. A real-vote guard is the path forward only if the threat model ever includes malicious members. |

> **Wave-3 dispatch note.** The harness double-dispatched three of these tasks (N-05, §E-1, §E-2/E-4). For each, one implementation was merged and the duplicate discarded; §E-1's discarded twin independently found the same HMAC double-prefix bug (ported above), and §E-2/E-4's discarded twin's broader voter-guard was the declined alternative noted above.
