...
- **Cluster:** quorum
- **Resolution:** Landed the EF-03 multi-node TCP integration test that R-039's unit fix unblocked. The test starts 3 in-process `TestNode` instances on ephemeral ports with RF=2, polls `committed_topology_members().len() == 3` (so `peak_cluster_size` advances to 3 via the `MembershipChanged` event chain), shuts down 2 of the 3 peers, then polls until SWIM has marked them dead (`node_addresses().len() <= 1` — `RunningCluster::new` at coordinator.rs:526 pre-seeds `self_addr` into `node_addrs`, so the surviving node retains *only* the self entry after both peers are NodeLeft-removed). At that point `alive_node_count() = 1`, `peak_cluster_size() = 3`, `quorum_needed = (3/2)+1 = 2`, and the test sends `OP_CREATE_BATCH` against the surviving node and asserts the response is `STATUS_ERROR` carrying `ERR_NO_QUORUM` (15) in the first 2 bytes of the payload (matching `error_response()`'s `[code:u16][msg_len:u16][msg:N]` layout). The control test starts a single-node RF=1 cluster (`peak_cluster_size <= 1`, the `peak <= 1` early return in `check_quorum`) and asserts the same `OP_CREATE_BATCH` returns `STATUS_OK`. This pins the contract that quorum rejection fires for an isolated remnant of a previously-multi-node cluster but does NOT spuriously reject in standalone deployments. As a side fix the commit applies project-wide `cargo fmt` (45 pre-existing fmt drifts that would have failed the local gate) and collapses 2 nested `if let / if` blocks in `src/device.rs` test helpers that triggered `clippy::collapsible-if` and would have failed `cargo clippy --all --all-targets -- -D warnings`. None of the production code paths changed; only test-helper formatting and the new integration tests.
...
- **Verification:** Full local gate green: `cargo build --release` clean, `cargo test --all` 1475 lib + 23 cluster_tcp + remaining suites pass (one pre-existing flake in `cluster::migration::tests::migration_active_gauge_tracks_inflight_shards` which races on the `migration_metrics()` singleton in parallel runs and is reproducible on the unmodified baseline; unrelated to R-040), `cargo clippy --all --all-targets -- -D warnings` clean, `cargo fmt --all -- --check` clean.
