...
- **Severity:** HIGH
- **Status:** RESOLVED
- **Files:** src/storage/blobstore.rs (`BlobStore::list` enumerator + `FileBlobStore` walk + stale `.tmp` sweep), src/storage/blob_gc.rs (new module: `reconcile_orphan_blobs_with` generic core, `reconcile_orphan_blobs_against_index` recovery wrapper, `reconcile_orphan_blobs` engine wrapper, `BlobGcConfig`/`spawn_blob_gc_task` background thread), src/storage/mod.rs (export), src/recovery.rs (`reconcile_blobs_after_recovery` startup hook), src/bin/server.rs (run reconciliation immediately after `recover_all_with_allocator` + spawn the periodic sweep alongside the checkpoint task), src/config.rs (`blob_gc_interval_secs`, default 3600, 0 disables periodic), tests/blob_gc_recovery.rs (audit-prescribed integration tests).
- **Cluster:** blob-gc
- **Resolution:** Pre-fix every failed create / aborted upload / cancelled migration leaked an external blob to disk forever — the foreground pipeline never re-read or cleaned a payload whose primary-index registration didn't land, and there was no enumeration entry-point on `BlobStore` so a janitor was impossible to write. The fix lands in three layers: (1) `BlobStore::list() -> Result<Vec<[u8; 32]>>` is now part of the trait. `FileBlobStore::list` walks the prefix tree (recursive, fault-tolerant — single-entry stat failures are logged and skipped so the sweep can make forward progress), returning every txid whose payload AND sidecar are present (matching the existing `exists` contract, so half-written blobs are not surfaced to the GC reconciler). As a side effect of the walk it also deletes any `.tmp` file whose mtime is older than `FileBlobStore::STALE_TMP_AGE_SECS` (5 minutes — the upload window must be tighter than that). `MemoryBlobStore::list` returns its keys. (2) `storage::blob_gc::reconcile_orphan_blobs_with` is generic over the index-lookup closure so the same logic is shared by recovery (closure borrows `&PrimaryBackend`) and the runtime sweep (closure borrows the engine). A blob is orphan when its primary-index entry is absent OR present without `TxFlags::EXTERNAL` (audit's exact definition). Per-blob delete failures are logged and counted in `BlobGcStats::delete_failed`; the sweep cannot be wedged by a single stuck blob. (3) `recovery::reconcile_blobs_after_recovery` runs immediately after `recover_all_with_allocator` returns from the startup path, while no client is connected — the concurrency race a runtime sweep can hit (deleting a freshly-uploaded blob whose registration is about to land) is structurally impossible at recovery time. The periodic sweep (`spawn_blob_gc_task` in `src/bin/server.rs`, alongside the existing checkpoint task) defaults to a 1-hour cadence via the new `blob_gc_interval_secs` config field; setting it to 0 disables the periodic sweep while leaving recovery-time reconciliation in place.
- **Tests added (in `tests/blob_gc_recovery.rs`):** `failed_create_blob_garbage_collected_on_recovery` (audit-prescribed: writes a blob, never registers the index entry, asserts recovery deletes it and observes `deleted_no_index = 1`), `blob_gc_keeps_blobs_referenced_by_external_flagged_records` (regression guard against an over-eager GC nuking a valid `EXTERNAL`-flagged blob; also round-trips `get` to confirm the kept payload still digest-verifies), `blob_gc_skips_blobs_not_in_primary_index` (multi-orphan: three blobs with no index entries are all reclaimed), `blob_gc_deletes_blobs_when_index_entry_missing_external_flag` (record exists but `tx_flags` lacks `EXTERNAL` → blob is debris from a create that ended up using the inline / separate tier and is reclaimed), `blob_gc_mixed_set_recovery` (interleaving of all three categories in one `BlobStore::list` pass), `stale_tmp_files_swept_on_recovery` (uses `filetime` to backdate a `.tmp` past `STALE_TMP_AGE_SECS` and asserts it is swept while a fresh in-flight `.tmp` survives), `reconcile_orphan_blobs_against_index_smoke` (direct-call regression baseline for the lower-level entry point). Plus 4 unit tests in `src/storage/blob_gc.rs` (`reconcile_keeps_external_blobs`, `reconcile_deletes_blob_with_no_index_entry`, `reconcile_deletes_blob_when_index_entry_lacks_external_flag`, `reconcile_mixed_set`) and 5 unit tests in `src/storage/blobstore.rs` (`file_list_returns_finalised_blobs_only`, `file_list_skips_meta_and_unrelated_files`, `file_list_sweeps_stale_tmp_files`, `memory_list_returns_keys`, `memory_list_after_delete`). Regression-proven: temporarily forcing `reconcile_orphan_blobs_with` to keep every blob makes `failed_create_blob_garbage_collected_on_recovery` and `blob_gc_skips_blobs_not_in_primary_index` fail with the expected `deleted_no_index` mismatch; setting the lookup closure to always return `LookupOutcome::NoEntry` makes `blob_gc_keeps_blobs_referenced_by_external_flagged_records` fail.
- **Verification:** `cargo build --release` clean; `cargo clippy --all --all-targets -- -D warnings` clean; `cargo fmt --all -- --check` clean; `cargo test --all` runs the 7 new integration tests (all pass) plus the 4+5 new unit tests with no regression to existing suites.
- **Test:** `failed_create_blob_garbage_collected_on_recovery`, `blob_gc_keeps_blobs_referenced_by_external_flagged_records`, `blob_gc_skips_blobs_not_in_primary_index`, `stale_tmp_files_swept_on_recovery`
