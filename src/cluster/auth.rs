...
//! | `OP_MIGRATION_WEAK_VETO_ARBITRATE` (245) | W13 — a VETO TICKET, minted only by this node's own completion refusal for that `(shard, source, key)` and CONSUMED on redemption (`cluster::migration::WeakVetoTicketStore`). The first delivery redeems the ticket; a replay finds nothing outstanding and is refused, so re-delivery is a no-op rather than a second override. Tickets additionally expire after `WEAK_VETO_TICKET_TTL` (60 s), well inside this layer's +/-5 min timestamp window, so a frame captured and replayed near the edge of that window cannot redeem even a never-used ticket. The frame is further gated on the epoch equalling this node's currently-activated shard-table version, and clears nothing on a `ClientDelete`/`Dah`/unknown-cause marker regardless. |
