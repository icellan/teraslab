...
//! |---|---|
//! | `OP_REPLICA_BATCH` (240) | Per-stream applied-sequence journal (`ReplicaAppliedTracker`) skips any batch whose sequence ≤ `last_applied_seq`; below that, the per-record generation guard + create-payload dedup absorb re-deliveries. CAVEAT: migration-flagged and out-of-band (`first_sequence == 0`) batches bypass the journal entirely, and `Delete`/`PruneSlot`/`PruneSlotIfSpentBy`/`RemoveConflictingChild` carry no generation — a replayed OOB frame containing one re-applies with effect inside the skew window. The stale-epoch gate's acceptance arm deliberately refuses to vouch for those variants (`stale_batch_from_current_master`'s vouchable rule), so the residual is the pre-existing same-epoch replay, accepted as-is per this table's escape clause. |
