...
//! |---|---|
//! | `OP_REPLICA_BATCH` (240) | Per-record generation guard + create-payload dedup + engine-level idempotent re-apply (idempotent re-spend, no-op unspend) absorb re-deliveries. The per-stream applied-sequence journal (`ReplicaAppliedTracker`) provides gap DETECTION only — since issue #17 it never skips a batch at/below `last_applied_seq` (a covered position does not prove that batch's content was applied; the sender relabels after a Gap NAK, so skipping silently dropped acked writes). CAVEAT: migration-flagged and out-of-band (`first_sequence == 0`) batches bypass the journal entirely, and `Delete`/`PruneSlot`/`PruneSlotIfSpentBy`/`RemoveConflictingChild` carry no generation — a replayed frame (OOB or tracked) containing one re-applies with effect inside the skew window. The stale-epoch gate's acceptance arm deliberately refuses to vouch for those variants (`stale_batch_from_current_master`'s vouchable rule), so the residual is the pre-existing same-epoch replay, accepted as-is per this table's escape clause. |
...
//! | `OP_MIGRATION_WEAK_VETO_ARBITRATE` (245) | W13 — a VETO TICKET, minted only by this node's own completion refusal for that `(shard, source, key)` **on that connection**, and CONSUMED on redemption (`server::ConnectionState::issue_weak_veto_ticket`). The first delivery redeems the ticket; a replay on the same connection finds nothing outstanding, and a replay on any OTHER connection never had a ticket at all — so re-delivery is a no-op rather than a second override. Because tickets are connection-scoped and a connection is handled serially by one thread, the mint/redeem pair needs no cross-thread serialisation and the consume is not racy. The frame is further gated on the epoch equalling this node's currently-activated shard-table version, and suspends nothing on a `ClientDelete`/`Dah`/unknown-cause marker regardless. NOTE the ticket is a replay/containment property, NOT proof of an in-flight transfer: the completion's `from_node` is self-asserted, so a frame-capable peer can mint one on demand — see the opcode doc. |
