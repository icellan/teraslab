...
/// `unregister_batch`, etc.) performs a read-modify-write sequence whose
/// `self.count` cache is
...
impl RedbPrimary {
    ...
    /// (`register`, `unregister`, `unregister_batch`, etc.) returns an
    /// [`IndexError`] instead of performing the write. The
...
mod tests {
    ...
    fn insert_1000_lookup_all() {
        let (_dir, mut primary) = open_temp();
        for i in 0..1000u64 {
            primary.register(make_key(i), make_entry(i * 104)).unwrap();
        }
    ...
    fn streaming_iterator_does_not_materialize_full_set() {
        ...
        assert!(first.1.record_offset < total * 104);
        assert!(
        ...
        for (_key, entry) in iter {
            assert!(entry.record_offset < total * 104);
            count += 1;
        }
    ...
    fn stats_report() {
        ...
        for i in 0..10u64 {
            primary.register(make_key(i), make_entry(i * 8)).unwrap();
        }
        let stats = primary.stats();
    ...
    fn register_overwrite_same_key() {
        let (_dir, mut primary) = open_temp();
        let key = make_key(1);
        primary.register(key, make_entry(104)).unwrap();
    ...
    fn is_empty_transitions() {
        ...
        primary.register(make_key(1), make_entry(104)).unwrap();
        assert!(!primary.is_empty());
    ...
    fn unregister_then_reregister() {
        ...
        assert_eq!(e.record_offset, 10000);
    }
    ...
    fn register_batch_basic() {
        ...
        let entries: Vec<_> = (0..100u64)
            .map(|i| (make_key(i), make_entry(i * 104)))
            .collect();
    ...
    fn register_batch_with_duplicates() {
        ...
        let entries = vec![
            (make_key(1), make_entry(104)),
            (make_key(1), make_entry(200)),
            (make_key(1), make_entry(304)),
        ];
        primary.register_batch(&entries).unwrap();

        assert_eq!(primary.len(), 1);
        let e = primary.lookup(&make_key(1)).unwrap().unwrap();
        assert_eq!(e.record_offset, 304);
    }
    ...
    fn register_batch_matches_individual() {
        ...
        for i in 0..50u64 {
            primary1.register(make_key(i), make_entry(i * 104)).unwrap();
        }
    ...
    fn unregister_batch_basic() {
        ...
        assert_eq!(results[0].unwrap().record_offset, 104);
        assert_eq!(results[1].unwrap().record_offset, 208);
        assert_eq!(results[2].unwrap().record_offset, 312);
    ...
    fn unregister_batch_with_missing_keys() {
        let (_dir, mut primary) = open_temp();
        primary.register(make_key(1), make_entry(104)).unwrap();
        primary.register(make_key(3), make_entry(304)).unwrap();
    ...
    //
    // Before the fix, unregister_batch silently swallowed redb commit errors
    // and returned vec![None; N], making callers believe the keys were
    // missing when they were actually still present.
    //
